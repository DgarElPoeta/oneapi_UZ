...
// Function that writes in standar output the filter
void print_filter(std::string name, std::vector<ftype>& f, uint64_t N) {
  std::cout << name << "\n";
...
// Function that writes in standar output the image
      ...
      for(size_t x = 0; x < filterDim; x++){
        for(size_t y = 0; y < filterDim; y++){
          ...
          if(r >= 0 && r < N && c >= 0 && c < N){
            ...
            ftype weight  = g.filter[x * filterDim + y];
            p += aux * weight;
          }
        }
      }
...
int main(int argc, char *argv[]) {
  ...
  for(size_t i = WORK_GROUP_SIZE; i < N*N; i*=2){
    ...
    std::memcpy(opts.pData.input.data() + i, opts.pData.input.data(), chunk_size * sizeof(ptype));
  }
