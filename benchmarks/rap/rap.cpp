...
int usage(const std::string &name) {
  std::cerr
      << "Usage:\n" << name << " <cpu|fpga|cpu_fpga> <static|dynamic|hguided> <num pkgs (dyn)|cpu proportion (st|hg)> <problem size> <M size>\n"
...
// Function that writes in standar output the vector
void print_vec(std::string name, std::vector<ptype>& m, uint64_t N) {
  std::cout << name << "\n";
...
bool verify(uint64_t N, uint64_t M,  std::vector<ptype>& a, std::vector<ptype>& b, std::vector<ptype>& func) {
  bool verification_passed = true;

  for (size_t i = 0; i < M; ++i) {
    ptype host_value = func[0];
    ptype kernel_value = b[i];
    for(size_t j = 0; j <= i; j++){
      ptype aux = a[i-j] + func[j];
      host_value = std::max(aux,host_value);
    }
    if(host_value != kernel_value){  
      std::cerr << "VERIFICATION FAILED for element " << i << "\n\tThe next statement isn't true: kernel_value == host_value \n\t-> " << kernel_value 
                << "!=" << host_value << "\n";
...
int main(int argc, char *argv[]) {
  ...
  // Problem M arg
  int64_t m = atoi(argv[5]);
  if(m < 0 || m >= N){
    std::cerr << "M variable must be greater or equal to 0 and lower than problem size " << "\n\n";
    return usage(name);
  }
  const uint64_t M = m;
  ...
  // Initialization of Rap data type of Opts
  opts.pData = Rap();
  opts.pData.size = N;
  opts.pData.M = M;
  opts.pData.a = std::vector<ptype>(N);
  opts.pData.b = std::vector<ptype>(N);
  opts.pData.func = std::vector<ptype>(N);
  ...
  // -------------------------------------------------------------------------------------------------
  // Initialization of elements in matrices in Rap data type of Opts with random values

  constexpr ptype nMin = -100, nMax = 100;
  std::random_device dev;
  std::mt19937 gen(dev()); 
  std::uniform_int_distribution<ptype> dis(nMin,nMax);
  for (size_t i = 0; i < WORK_GROUP_SIZE; i++) {
    opts.pData.a[i] = dis(gen);
    opts.pData.func[i] = dis(gen);
  }
  ...
  for(size_t i = WORK_GROUP_SIZE; i < N; i*=2){
    ...
    std::memcpy(opts.pData.func.data() + i, opts.pData.func.data(), chunk_size * sizeof(ptype));
  }
  ...
  if (mode == Mode::CPU) {
    ...
    for(size_t i=1; i<num_cpu_threads; i++){
      ...
      vecOfThreads[i-1] = std::thread(process<Rap>, true, std::ref(opts), i);
    }
    ...
    for(size_t i=0; i<num_cpu_threads; i++){
      ...
      // Create thread with CPU scheduler process
      vecOfThreads[i] = std::thread(process<Rap>, true, std::ref(opts), i);
    }
  ...
  if (check) {
    ...
    if (verify(N, M, opts.pData.a, opts.pData.b, opts.pData.func)) {
  ...
  if(print){
    print_vec("A", opts.pData.a, N);
    print_vec("FUNC", opts.pData.func, N);
    print_vec("B", opts.pData.b, N);
  }
