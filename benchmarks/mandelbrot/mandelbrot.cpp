...
void print_image(std::string name, std::vector<ptype>& image, uint64_t N, uint64_t max_iterarions) {
  ...
  for (size_t i = 0; i < N; ++i) {
    for (size_t j = 0; j < N; ++j) {
      std::string s = (image[i * N + j] < max_iterarions) ? "-" : "*";
      std::cout << s << " ";
    }
    std::cout << "\n";
  }
...
bool verify(Mandelbrot& m) {
  ...
  const auto N = m.size;
  const auto max_iterations = MAXITERATIONS;

  for (size_t i = 0; i < N; ++i) {
    for (size_t j = 0; j < N; ++j) {
      
      const auto kernel_value = m.image[i * N + j];
      const ctype c = ctype(MINX + (i * MAXX / N), MINY + (j * MAXY / N));
      ctype z = 0;
      float abs_z_square = 0;
      uint64_t iter = 0;
      for(iter = 0; iter < max_iterations && abs_z_square < 4.0f; ++iter){
      ...
      if(iter != max_iterations) iter--;

      const auto host_value = iter;

      const auto difference = (kernel_value > host_value) ? (float) host_value / kernel_value : (float) kernel_value / host_value;
      if (difference < 0.5) {
...
int main(int argc, char *argv[]) {
  ...
  // Initialization of Mandelbrot data type of Opts
  opts.pData = Mandelbrot();
  opts.pData.size = N;
  opts.pData.image = std::vector<ptype>(N*N,0);
  ...
  if (mode == Mode::CPU) {
    ...
    for(size_t i=1; i<num_cpu_threads; i++){
      ...
      vecOfThreads[i-1] = std::thread(process<Mandelbrot>, true, std::ref(opts), i);
    }
    ...
    process<Mandelbrot>(true, opts, 0);
    ...
    process<Mandelbrot>(false, opts, 0);
    ...
    for(size_t i=0; i<num_cpu_threads; i++){
      ...
      // Create thread with CPU scheduler process
      vecOfThreads[i] = std::thread(process<Mandelbrot>, true, std::ref(opts), i);
    }
  ...
  // Type of benchamrk
  std::cout << "Benchmark: mandelbrot\n";
  std::cout << "Image size: " << N << " x " << N << "\n";
  std::cout << "Problem size: " << N << " (an entire row of the image is considered the work unit)\n";
  std::cout << "\n\n";
