...
int usage() {
  std::cout
      << "usage: <cpu|gpu|cpugpu> <static|dynamic> <num pkgs (dyn)|gpu proportion (st|hg)> <size side> [num_cpp_threads]\n"
      << "DEBUG=y\n"
      << "CHECK=y\n"
...
int main(int argc, char *argv[]) {
  ...
  printf("Timepo inicialización: %f\n", (std::chrono::high_resolution_clock::now() - tStart).count() / 1e6);
  ...
  *opts.p_rest_size = problem_size;
  ...
  if (mode == Mode::CPU || mode == Mode::CPUGPU) {
    printf("Device id: 1\n");
    printf("Selected device: c++ nativo\n");
